...
export async function getAdminPortalSummary(): Promise<AdminPortalSummary> {
  ...
  const [
    ...
    admin
      ...
      .select("amount_mmk")
      .eq("status", "approved"),
    admin.from("user_hsk_access").select("user_id"),
  ]);
  ...
  if (errors.length > 0) {
    throw new Error(errors[0]?.message ?? "Failed to load admin summary.");
  }

  const totalUsers = usersResult.count ?? 0;
  const paidUsers = new Set(
    (accessResult.data ?? []).map((row) => String(row.user_id)),
  ).size;
  ...
  return {
    ...
    totalSalesMmk: (salesResult.data ?? []).reduce(
      (total, row) => total + Number(row.amount_mmk ?? 0),
      0,
    ),
  };
}

export async function getAdminUsers(): Promise<AdminUserAccessRow[]> {
  const admin = createSupabaseAdminClient();

  const [profilesResult, authResult, accessResult] = await Promise.all([
    admin
      .from("profiles")
    ...
    admin
      ...
      .select("user_id,product_code,level,lifetime,granted_at")
  ...
  for (const row of accessResult.data ?? []) {
    ...
    const list = accessMap.get(userId) ?? [];
  ...
  return (profilesResult.data ?? []).map((profile) => {
    ...
    return {
      id,
      email: String(profile.email ?? authUser?.email ?? "No email"),
      name:
